...
/* We use an explicit function pointer here so we can reestablish
   a saved interrupt-handler address (such a variable would need
   type pointer-to-function, and would not match a parameter type
   of IntHandler here--another C gotcha.  At least we can say
   const to make it clear that this is an input argument) */
