/* SAPC stdio.h, modified to de-support lots of C lib functions,
   leaving sscanf, printf, kprintf, vsprintf, and a few basic decls 
   for var args, etc. */
...
/* SAPC: from libio.h. size_t is a type used in many library functions */
typedef unsigned int size_t;

/* SAPC: from libio.h. Support both ANSI and pre-ANSI C by
   uing this macro in each prototype of library functions */
#ifndef __P
#ifdef __STDC__
...
/* SAPC: library prototypes */
#ifndef REAL
extern int fgetc __P((int dev));
extern int fputc __P((int, int dev));
extern int fputs __P((__const char *str, int dev));
...
#ifdef XINU
/* funny non-C-library order of args */
extern char* fgets __P((int dev, char *, int));
#else
extern char* fgets __P((char *, int, int dev));
#endif

/* SAPC: special function for printf without interrupt on */
extern int kprintf __P((__const char* format, ...));
#endif
